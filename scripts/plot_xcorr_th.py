...
for iconfig, config_path in enumerate(config_paths):
    ...
    autoits_split = np.split(autoits, len(selected), axis = -1)
    crosses_split = np.split(crossits, len(selected), axis = -1)
    crossits_down_split = np.split(crossits_down, len(selected), axis = -1)
    ...
    if iconfig == 0:
        ...
        nrows = 1
        ncols = 3
        fig, axes = plt.subplots(nrows=nrows, ncols = ncols, figsize = (5*ncols, 3), squeeze = False, sharex = True)
        plt.subplots_adjust(hspace=0.4)

    for i, k in enumerate(plotting):
        ...
        for s in range(nrows):
            ...
            for j, itr in enumerate([0, itmax]):
                if (s == 0) and (i == 0):
                    ...
                    xcorrs = np.array([ud.cross_corr_coeff_from_cl(autoits_split[original_idx][simidx, itr, :], auto_input_its[simidx, original_idx], crosses_split[original_idx][simidx, itr, :], plot=False, bin = False)[1] for simidx in range(autoits.shape[0])])
                    std = xcorrs.std(axis = 0)/np.sqrt(xcorrs.shape[0])
                    mean = xcorrs.mean(axis = 0)
                    #print(mean/cross)
                    #ax.plot(el, mean, color = "black")
                    #ax.fill_between(el, mean-std, mean+std, color = "black", alpha = 0.4)
                    #if (k == "a") and (s == 0):
                    #        np.savetxt(f"crossa_iconfig_{iconfig}_itr_{itr}.txt", np.c_[el, cross])
                    make_label = (((i == 0) and (iconfig == 0)))
                    ax.plot(el, mean, label = caso_mf[itr] if make_label else None, color = colors[1] if itr != 0 else colors[0], ls = "-")#, zorder = 1)
                    ax.fill_between(el, mean-std, mean+std, color = colors[1] if itr != 0 else colors[0], alpha = 0.3)
                    
                    if (k == "a"):
                        rho_QE, rho_MAP = ud.get_rhos(field = "a")
                    ...
                    el, cross = ud.cross_corr_coeff_from_cl(autos_split[original_idx][itr], inputs[ud.get_down_index(selected, k, 1)], crosses_downs[0][original_idx][itr], plot=False, bin = False)
                    xcorrs = np.array([ud.cross_corr_coeff_from_cl(autoits_split[original_idx][simidx, itr, :], auto_input_its[simidx, ud.get_down_index(selected, k, 1)], crossits_down_split[original_idx][simidx, itr, :], plot=False, bin = False)[1] for simidx in range(autoits.shape[0])])
                    std = xcorrs.std(axis = 0)/np.sqrt(xcorrs.shape[0])
                    mean = xcorrs.mean(axis = 0)
                    ax.plot(el, mean, label = None, color = colors[1] if itr != 0 else colors[0], ls = "-")#, zorder = 1)
                    ax.fill_between(el, mean-std, mean+std, color = colors[1] if itr != 0 else colors[0], alpha = 0.4)

                    rho_contaminated_QE, rho_contaminated_MAP, rho_contaminated_MAP_derot = ud.get_rhos_omega(field = "a")
                    rho_contaminated_QE_no_n1, _, _ = ud.get_rhos_omega(field = "a", n1 = False)
                    ax.plot(rho_contaminated_QE, color = colors_2[3])
                    ax.plot(rho_contaminated_MAP, color = colors_2[1])
                    ax.plot(rho_contaminated_MAP_derot, lw = 2, color = colors_2[2])

                elif (k == "o"):
                    print(original_idx, ud.get_down_index(selected, k, 1))
                    el, cross = ud.cross_corr_coeff_from_cl(autos_split[original_idx][itr], inputs[ud.get_down_index(selected, k, 1)], crosses_downs[0][original_idx][itr], plot=False, bin = False)
                    ...
                    if (iconfig>0) and (itr > 0):
                        ax.plot(el, mean, label = "MAP delensing only", color = colors[4+j+s])
                        ax.fill_between(el, mean-std, mean+std, color = colors[4+j+s], alpha = 0.4)
                        ax.legend(fontsize = 14)
                        rho_contaminated_QE, rho_contaminated_MAP, rho_contaminated_MAP_derot = ud.get_rhos_omega()
                        ax.plot(rho_contaminated_QE, color = colors_2[3])
                        ax.plot(rho_contaminated_MAP, color = colors_2[1])
                        ax.plot(rho_contaminated_MAP_derot, color = colors_2[2])
                        #ax.set_ylim(-1, 0)
            ...
            if (s >= 1) or (i>0):
                ax.set_title(f"{ud.generate_hat_bracket(k, selected[ud.get_down_index(selected, k, 1)])}", fontsize=22)
            else:
...
#fig.tight_layout()
plt.savefig(f"{ud.out_dir}xcorr_theory_{ud.nome[sindex]}_{savingname}.pdf")
