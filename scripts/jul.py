...
noise_phas = phas.lib_phas(opj(os.environ['SCRATCH'], 'noisephas_lmax%s'%4096), 3, 4096) # T, E, and B noise phases
cmb_phas = phas.lib_phas(opj(os.environ['SCRATCH'], 'cmbphas_lmax%s'%5120), 5, 5120) # unlensed T E B P O CMB phases
...
# for delensing useful to cut much more B. It can aclso help since the cg inversion does not have to reconstruct those.
...
def get_itlib(k:str, simidx:int, version:str, cg_tol:float):
    ...
    np.save("/capstor/scratch/cscs/odarwish/delensalot_idealized_curly/ptt_sim0000/plm0_norm.npy", plm0)
